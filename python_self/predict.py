...
from log import mylog,mylog_section
...
_buckets =[(120, 30), (200, 35), (300, 40), (400, 41), (500, 42)]
    ...
    with tf.Session(config=config) as sess:
        ...
        for source_inputs, bucket_id, length in ite:
            ...
            for i in xrange(max_target_length):
                if i == 0:
                    top_value, top_index, eos_value = model.beam_step(sess, bucket_id, index=i, sources=source_inputs,
                                                                      target_inputs=target_inputs)
                else:
                    top_value, top_index, eos_value = model.beam_step(sess, bucket_id, index=i,
                                                                      target_inputs=target_inputs,
